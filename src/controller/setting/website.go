...
func WebsiteConfigList(c *gin.Context) {
	var db = database.DB
	method := c.Request.Method
	if method == "GET" {
		var dataList []model.WebsiteConfig
	...
	if method == "POST" {
		var record model.WebsiteConfig
		c.BindJSON(&record)
	...
	if method == "DELETE" {
		var record model.WebsiteConfig
		c.BindJSON(&record)
		result := database.DB.Model(&model.WebsiteConfig{}).Where("id = ?", record.Id).Delete(record)
		if result.Error != nil {
