...
func DoQuery(c *gin.Context) {
	...
	if queryType == "execute" && (datasourceType == "MySQL" || datasourceType == "TiDB" || datasourceType == "Doris" || datasourceType == "MariaDB" || datasourceType == "GreatSQL" || datasourceType == "PostgreSQL" || datasourceType == "Oracle" || datasourceType == "ClickHouse") {
		...
		if admin != true {
			...
			if sqlType == "create" || sqlType == "alter" {
				...
				if !checkResult {
					WriteLog(username.(string), datasourceType, datasource, queryType, sqlType, databaseName, intercept, 0, sql, fmt.Sprintf("验证权限失败，%s", msg))
					c.JSON(http.StatusOK, gin.H{"success": false, "msg": fmt.Sprintf("验证权限失败，%s", msg)})
					return
				}
			}
			//select/insert/update循环表检查权限
			for _, table := range queryTableList {
				...
				checkResult, msg = CheckPrivilege(username.(string), datasourceType, datasource, databaseName, table, sqlType, queryNumber)
				if !checkResult {
					WriteLog(username.(string), datasourceType, datasource, queryType, sqlType, databaseName, intercept, 0, sql, fmt.Sprintf("验证权限失败，%s", msg))
					c.JSON(http.StatusOK, gin.H{"success": false, "msg": fmt.Sprintf("验证权限失败，%s", msg)})
					return
		...
		//如果是update/delete，根据where条件转换成备份sql
		if queryType == "execute" && (sqlType == "update" || sqlType == "delete") {
			findWhereInfo := regexp.MustCompile(`(?i:where).*$`).FindAllStringSubmatch(sql, -1)
			doTable := findQueryTable[0][1]
			doWhere := findWhereInfo[0][0]
			backupSql = strings.Replace(fmt.Sprintf("select * from %s %s", doTable, doWhere), ";", "", -1)
			backupName = fmt.Sprintf("%s.%s.%s.txt", databaseName, doTable, time.Now().Format("20060102150405"))
		}
	...
	if datasourceType == "MySQL" || datasourceType == "TiDB" || datasourceType == "Doris" || datasourceType == "MariaDB" || datasourceType == "GreatSQL" || datasourceType == "OceanBase" {
		...
		defer dbCon.Close()
		//update/delete执行前先备份数据
		if queryType == "execute" && dmlBackupEnable == "1" && (sqlType == "update" || sqlType == "delete") {
			mysqlBackupSql := fmt.Sprintf("%s into outfile %s/%s fields terminated by ','", backupSql, dmlBackupDir, backupName)
			_, err = db.QueryAll(dbCon, mysqlBackupSql)
			if err != nil {
				WriteLog(username.(string), datasourceType, datasource, queryType, sqlType, databaseName, failed, 0, sql, fmt.Sprintf("导处备份数据失败，数据变更终止：%s", err))
				c.JSON(http.StatusOK, gin.H{"success": false, "msg": fmt.Sprintf("导处备份数据失败，数据变更终止：%s", err)})
				return
			}
