...
/** 与旧版 web 一致：中文名优先，否则登录名-YYYYMMDD */
function getDefaultWatermarkText() {
  const info = userStore.userInfo;
  const name = info?.realName || info?.username || '用户';
  return `${name}-${dayjs().format('YYYYMMDD')}`;
}
  ...
  <BasicLayout @clear-preferences-and-logout="handleLogout">
    <template #user-dropdown>
      <UserDropdown
        :avatar
        :menus
        :text="userStore.userInfo?.username || userStore.userInfo?.realName"
