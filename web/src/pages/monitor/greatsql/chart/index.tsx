...
const DemoLine: React.FC = () => {
  ...
  useEffect(() => {
    setCurrent("chart");
    fetch('/api/v1/meta/node/list_search?module=greatsql')
  ...
  const asyncFetch = (values: {}) => {
    ...
    fetch('/api/v1/performance/greatsql/chart', {
      method: 'post',
      headers: headers,
    ...
    <>
      <Menu mode="horizontal" selectedKeys={[current]} >
        <Menu.Item key="dashboard" icon={<MailOutlined />}>
          <a href="/performance/greatsql/health" rel="noopener noreferrer">
            GreatSQL健康大盘
          </a>
        </Menu.Item>
        <Menu.Item key="chart" icon={<AppstoreOutlined />}>
          <a href="/performance/greatsql/chart" rel="noopener noreferrer">
            GreatSQL性能图表
          </a>
        </Menu.Item>
      </Menu>
        ...
        >
          <Space>
            <Form.Item
              name={'instance'}
              label={'选择GreatSQL实例'}
              rules={[{ required: true, message: '选择GreatSQL实例' }]}
            >
        ...
        <>
          <Row gutter={[16, 24]}>
            ...
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.threadsChartList} unit="" />
                </div>
              </Card>
            </Col>
          </Row>
          <Row gutter={[16, 24]}>
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.queriesChartList} unit="" />
                </div>
              </Card>
            </Col>
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.slowQueriesChartList} unit="" />
                </div>
              </Card>
            </Col>
          </Row>
          ...
          <Row gutter={[16, 24]}>
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.dmlChartList} unit="" />
                </div>
              </Card>
            </Col>
            <Col span={12}>
          ...
          <Row gutter={[16, 24]}>
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.innodbChartList} unit="" />
                </div>
              </Card>
            </Col>
            <Col span={12}>
              <Card style={{ marginTop: 10 }}>
                <div style={{ height: 300 }}>
                  <LineChart data={data.bytesChartList} unit="kb" />
                </div>
              </Card>
            </Col>
          </Row>
