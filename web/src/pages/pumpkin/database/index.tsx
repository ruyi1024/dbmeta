...
const DatabaseCapacity: React.FC = () => {
  const [loading] = useState<boolean>(false);

  // 数据库容量信息查询表格列定义
  const databaseCapacityColumns: ProColumns<DatabaseCapacity>[] = [
    {
      title: '数据库名',
  ...
  // 获取数据库容量数据（用于表格，支持分页、搜索、排序）
  const fetchDatabaseCapacity = async (params: any, sorter: any) => {
    try {
      // 构建查询参数
      ...
      const response = await fetch(`/api/v1/pumpkin/capacity/database/top10?${queryParams.toString()}`);
      const json = await response.json();
      if (json.success && json.data) {
        const data: DatabaseCapacity[] = json.data.map((item: any) => {
      ...
      console.error('获取数据库容量数据失败:', error);
    }
    return {
      data: [],
      success: true,
  ...
  return (
    <div>
      ...
      >
        <ProTable<DatabaseCapacity>
          headerTitle={false}
          toolBarRender={false}
          rowKey="id"
          loading={loading}
          request={(params, sorter) => fetchDatabaseCapacity(params, sorter)}
          columns={databaseCapacityColumns}
          pagination={{
            pageSize: 10,
...
export default DatabaseCapacity;
